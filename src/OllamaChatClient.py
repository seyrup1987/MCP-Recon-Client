...
from OllamaModels import Models, Plan
...
logger = logging.getLogger("MCPClientSSE_Ollama")
...
class MCPClient:
    def __init__(self, max_tool_iterations=5, cache_size=1000, cache_ttl=3600):
        ...
        try:
            logger.info("Initializing MCP SSE Client with Ollama and Caching")
            self.config = Config()
            self.models = Models()
    ...
    def _estimate_tokens(self, messages: List[Dict[str, str]]) -> int:
        """Estimate token count for messages using a simple word-based approximation."""
        text = " ".join(m['content'] for m in messages if isinstance(m, dict) and 'content' in m)
        return len(text) // 4 + len(messages)
        ...
        if len(self.conversation_history) > 20:
            try:
                summary_prompt = "Summarize the following conversation concisely:\n" + \
                                "\n".join(m['content'][:200] for m in self.conversation_history[1:-10] if isinstance(m, dict) and 'content' in m)
                summary = await self.models.ainvoke([
                    {'role': 'system', 'content': "You are a summarizer. Provide a concise summary of the conversation."},
                    {'role': 'user', 'content': summary_prompt}
                ])
                self.conversation_history = (
                    [self.conversation_history[0]] +
                    [{'role': 'user', 'content': summary['message']['content']}] +
    ...
    def _truncate_history(self):
        ...
        if len(self.conversation_history) > self.max_history_messages:
            system_messages = [m for m in self.conversation_history if m.get('role') == 'system']
            other_messages = [m for m in self.conversation_history if m.get('role') != 'system']
    ...
    def _bind_tools_to_llm(self):
        """Bind the discovered MCP tools to the LLM with validation."""
        if not self.tools_schema_for_binding:
            logger.warning("No tool schemas found to bind to LLM.")
            raise ValueError("No tool schemas found to bind to LLM.")
        try:
            cleaned_schemas = []
            ...
            if not cleaned_schemas:
                ...
                raise ValueError("No valid tool schemas after validation.")
            self.models.initializeTools(cleaned_schemas)
    ...
    async def _invoke_llm(self, messages: List[Dict[str, str]]) -> Dict[str, Any]:
        """Invoke the LLM with manual async caching and retries."""
        @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
        async def invoke_with_retry():
            response = ic(await self.models.ainvoke(messages))
        ...
        if cached_value is not None:
            ...
            return cached_value
        logger.debug(f"LLM cache miss for key={cache_key}, invoking model")
    ...
    async def _execute_plan(self, ai_plan: Plan, executed_tool_calls: set, final_results: list):
        ...
        try:
            for step in ai_plan.plan:
                ...
                valid_history = [msg for msg in self.conversation_history if isinstance(msg, dict) and 'content' in msg and str(msg['content']).strip()]
                ai_response = ic(await self._invoke_llm(valid_history))
                self.conversation_history.append({
                    'role': 'assistant',
                    'content': ai_response['message']['content'],
                    'tool_calls': ai_response['message'].get('tool_calls', [])
                })
                self._truncate_history()
                ...
                if tool_calls:
                    logger.info(f"Processing {len(tool_calls)} tool call(s): {[tc.get('function', {}).get('name', 'unknown') for tc in tool_calls]}")
                    results_added_this_step = False
                    for tool_call in tool_calls:
                        print("DEBUG LOG | tool_call: ", tool_call)
                        tool_name = ic(tool_call.get("name"))
                        tool_args = ic(tool_call.get("args", {}))
                        tool_call_id = ic(tool_call.get("id"))
                        ...
                        if len(tool_result_content) > max_result_len:
                            logger.warning(f"Tool result for {tool_name} is too long ({len(tool_result_content)} chars), truncating.")
                        ...
                        self._truncate_history()
                        results_added_this_step = True
                        logger.debug(f"Appended tool message for call_id {tool_call_id}")
            ...
            last_ai_message = next((msg['content'] for msg in reversed(self.conversation_history) if msg.get('role') == 'assistant' and not msg.get('tool_calls')), None)
            if last_ai_message:
    ...
    async def process_query(self, query: str) -> str:
        ...
        if not self.conversation_history:
            ...
            self.conversation_history.append({'role': 'system', 'content': self.system_prompt_template})
        ...
        try:
            ...
            return final_results[0]  # Return the final result as a string
    ...
    async def chat_loop(self):
        ...
        logger.info("Starting interactive chat loop (SSE Client - Ollama)")
        print("\nMCP SSE Client (Ollama) Started!")
...
async def main():
    ...
    client = None
    try:
        logger.info("--- Starting MCP SSE Client Application (Ollama) ---")
        client = MCPClient()
